  ...
  <div id="eodiro-banner" :class="{ mini: appearMini }">
    <div class="banner">
      <transition name="bg-fade">
        <div v-if="!routeSwitch" :class="`background background--${hamlet0}`" />
      </transition>
      <transition name="bg-fade">
        <div v-if="routeSwitch" :class="`background background--${hamlet1}`" />
      </transition>
...
export default {
  ...
  mounted() {
    ...
    CEM.addEventListener('beforepageleave', this.$el, () => {
      ...
      bannerElm.style.cssText = `position: fixed; top: 0; transform: translateY(${top}px); -webkit-transform: translateY(${top}px);`
      disableScroll.on()
    })
