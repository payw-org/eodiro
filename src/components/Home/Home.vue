...
<template>
  <div id="home">
    ...
    <!-- <transition
        ...
        name="zoom"
    >-->
    <div class="start-box" :class="{appear: appear}">
    ...
    startMsg() {
      if (process.client) {
        if (window.localStorage.getItem('defaultUniversity')) {
          const defaultUniversity = JSON.parse(
            window.localStorage.getItem('defaultUniversity')
          )
...
#home {
  ...
  $cb: cubic-bezier(0.24, 0.49, 0.01, 0.99);
  ...
  .start-box {
    ...
    &.zoom-leave-active {
      ...
      transition: transform $transition-time $cb, opacity $transition-time/2 $cb,
        filter $transition-time $cb, background-color $transition-property,
        box-shadow $transition-property;
    ...
    .content-area {
      ...
      .main {
        ...
        .go-btn {
          margin: 2rem auto 0;
          transition: background-color $transition-time $cb,
            box-shadow $transition-time $cb;
          display: flex;
